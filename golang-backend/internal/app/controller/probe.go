...
func ProbeCreate(c *gin.Context) {
    var p struct{ Name string `json:"name" binding:"required"`; IP string `json:"ip" binding:"required"` }
    ...
    if err := dbpkg.DB.Create(&rec).Error; err != nil { c.JSON(http.StatusOK, response.ErrMsg("保存失败")); return }
    c.JSON(http.StatusOK, response.Ok(rec))
}

// POST /api/v1/probe/update {id, name, ip, status?}
func ProbeUpdate(c *gin.Context) {
    var p struct{ ID int64 `json:"id" binding:"required"`; Name string `json:"name"`; IP string `json:"ip"`; Status *int `json:"status"` }
    if err := c.ShouldBindJSON(&p); err != nil { c.JSON(http.StatusOK, response.ErrMsg("参数错误")); return }
    var rec model.ProbeTarget
    if err := dbpkg.DB.First(&rec, p.ID).Error; err != nil { c.JSON(http.StatusOK, response.ErrMsg("不存在")); return }
    if p.Name != "" { rec.Name = p.Name }
    if p.IP != "" { rec.IP = p.IP }
    if p.Status != nil { rec.Status = *p.Status }
    rec.UpdatedTime = time.Now().UnixMilli()
    if err := dbpkg.DB.Save(&rec).Error; err != nil { c.JSON(http.StatusOK, response.ErrMsg("保存失败")); return }
    c.JSON(http.StatusOK, response.Ok(rec))
}

// POST /api/v1/probe/delete {id}
func ProbeDelete(c *gin.Context) {
    var p struct{ ID int64 `json:"id" binding:"required"` }
    if err := c.ShouldBindJSON(&p); err != nil { c.JSON(http.StatusOK, response.ErrMsg("参数错误")); return }
...
func AgentReportProbe(c *gin.Context) {
    ...
    if err := dbpkg.DB.Where("secret = ?", p.Secret).First(&node).Error; err != nil { c.JSON(http.StatusOK, response.ErrMsg("节点不存在")); return }
    now := time.Now().UnixMilli()
    if len(p.Results) == 0 { c.JSON(http.StatusOK, response.OkNoData()); return }
    rows := make([]model.NodeProbeResult, 0, len(p.Results))
    for _, r := range p.Results {
        t := now
        if r.TimeMs != nil && *r.TimeMs > 0 { t = *r.TimeMs }
...
func NodeNetworkStats(c *gin.Context) {
    var p struct{ NodeID int64 `json:"nodeId" binding:"required"`; Range string `json:"range"` }
    if err := c.ShouldBindJSON(&p); err != nil { c.JSON(http.StatusOK, response.ErrMsg("参数错误")); return }
    // time window
    now := time.Now().UnixMilli()
    var windowMs int64
    switch p.Range {
    case "1h": windowMs = 3600 * 1000
...
func NodeNetworkStatsBatch(c *gin.Context) {
    ...
    // aggregate per node
    type stat struct{ Sum int; Cnt int; Latest *int; LatestTarget int64 }
    agg := map[int64]*stat{}
    for _, r := range rows {
        s := agg[r.NodeID]
    ...
    for nid, s := range agg {
        ...
        if m, ok := tmeta[s.LatestTarget]; ok { out[nid]["latestTarget"] = map[string]any{"id": s.LatestTarget, "name": m["name"], "ip": m["ip"]} }
    }
